...
### ⚠️ 발견된 핵심 이슈 — **정비 주기표가 90° 회전 인쇄**

본 프로젝트에서 가장 중요한 표인 **정기 점검 주기표**가 페이지에 세로로 회전되어 인쇄되어 있습니다.
(해당 페이지 텍스트 라인의 **64개 중 62개가 세로 방향**)

이 때문에 로더별로 결과가 극명하게 갈립니다.

| 로더 | 회전 표 처리 | 추출 결과 |
|---|:---:|---|
| **PyMuPDF** | ✅ 정상 | `엔진 오일 및 오일 필터` |
| pdfplumber | ❌ 글자 역순 | `터필 일오 및 일오 진엔` |
| pdftotext (CLI) | ❌ 한글 전멸 | 0자 |
...
| 항목 | 베이스라인 값 |
|---|---|
| **PDF 로더** | `PyMuPDF` (회전 표 정상 처리 — 사전 검증 결과) |
...
│   ├── CN7HEV_2026_ko_KR.pdf    # 아반떼 HEV
│   ├── CE1_2025_ko_KR.pdf       # 아이오닉6 (EV)
│   ├── NX4_2025_ko_KR.pdf       # 투싼 (디젤)
│   └── NH2_2026_ko_KR.pdf       # 넥쏘 (수소)
├── src/
│   ├── config.py             # ⭐ 공통 베이스라인 설정 (변수 통제)
│   ├── check_pdf.py          # 사전 검증 — PDF 추출 진단
...
| 문서 처리 | **PyMuPDF(기본)**, pypdf, pdfplumber |
